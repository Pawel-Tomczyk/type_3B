...
int main(int argc, char *argv[]) {
    ...
    while (true) {
        ...
        while (info.msg_qnum > 0) {
            msgrcv(msgid_matches, &product_m, sizeof(product_m), 1, 0);
            walet += product_m.price;
            msgctl(msgid_matches, IPC_STAT, &info);
            printf("Walet: %d\n", walet);
        }

        if (shm_prices->paper + shm_prices->tabaco <= walet) {
            product_p.mtype = 1;
            product_p.price = shm_prices->matches;
            product_t.mtype = 1;
            product_t.price = shm_prices->tabaco;
            walet -= product_p.price;
            walet -= product_t.price;
            msgsnd(msgid_matches, &product_p, sizeof(product_p), 0);
            msgsnd(msgid_tabaco, &product_t, sizeof(product_t), 0);
            printf("\tPaid:\n");
            printf("\tPaper: %d\n\tTabaco: %d\n", product_p.price, product_t.price);
            printf("\tWalet: %d\n", walet);
            printf("\tSmoking...\n");
            sleep(5);
        }
